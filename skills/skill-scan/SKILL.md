...
4. **Preflight scanner.** Verify `skills/skill-scan/scan.sh` is present and executable. If missing (sandbox edge case), fall back to inline Grep using the same HIGH/MEDIUM/LOW pattern library defined in `scan.sh` — never silently skip.
...
15. **Log** to `memory/logs/${today}.md` with an `### skill-scan` section: scope, exit status code, counts by severity, new/resolved counts, PR/issue IDs filed, report path.
