...
b. **Out of window** — if `days_remaining_v7 < 7` OR `days_remaining_v7 > 14` → record `OUT_OF_WINDOW`, no alert. (Under 7d is too late to dispatch `show-hn` thoughtfully; over 14d is too far out and trades on noisy projection data.)
...
- **Only fires inside the launch window.** Both gates (7-14 day projection AND Tue/Wed/Thu landing) must pass. A 5-day projection is too late to dispatch `show-hn` thoughtfully; a 21-day projection is too far out and trades on noisy projection data.
