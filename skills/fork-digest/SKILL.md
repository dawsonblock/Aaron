...
Read `memory/topics/fork-digest-state.json` if it exists. Schema:
...
Full report: https://github.com/${GITHUB_REPOSITORY}/blob/main/articles/fork-digest-${today}.md
```
...
Write `memory/topics/fork-digest-state.json`:
...
- Do not parse last week's article for week-over-week — use `memory/topics/fork-digest-state.json` only.
